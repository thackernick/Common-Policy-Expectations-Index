...
% Select the relevant columns for the 3 month analysis
FFF_3month = FFF_monthly(:, {'FF3Comdty'});
OIS_3month = OIS_monthly(:, {'USSOCCurncy'});
Bluechip_3month = Bluechip_monthly(:, {'RFF_FF_F1'});
EFFR = EFFR_monthly(:, {'Var2'});   

%% Convert FF3Comdty to double and subtract from 100 if needed
if ismember('FF3Comdty', FFF_3month.Properties.VariableNames)
    if ~isnumeric(FFF_3month.FF3Comdty)
        FFF_3month.FF3Comdty = str2double(FFF_3month.FF3Comdty);
    end
    FFF_3month.FF3Comdty = 100 - FFF_3month.FF3Comdty;
else
    warning('The column "FF3Comdty" does not exist in FFF_3month.');
end

%% Merge all
merged_data = synchronize(FFF_3month, OIS_3month, Bluechip_3month, EFFR);

% Create lag for 3-month analysis
EFFR_lag = merged_data.Var2(4:end);  % Lagging by 3 months
FedFundsFutures_lag = merged_data.FF3Comdty(1:end-3);
OIS_lag = merged_data.USSOCCurncy(1:end-3);
BlueChipSurvey_lag = merged_data.RFF_FF_F1(1:end-3);

lagged_data = timetable(merged_data.Date(4:end), EFFR_lag, FedFundsFutures_lag, OIS_lag, BlueChipSurvey_lag, ...
